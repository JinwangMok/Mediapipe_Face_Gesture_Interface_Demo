...
class Face_angle_stabilizer():
    def __init__(self, stabilize_time=3.):
        self.__stabilize_time:np.float64 = np.float64(stabilize_time)

        self.__accumulated_time:np.float64 = np.float64(0.)
        self.__last_frame_time:np.float64 = np.float64(0.)
    ...
    def stablize_angles(
        ...
        if self.__is_first_step():
            ...
            self.__last_frame_time = np.float64(time.time())
            return face_info
        ...
        self.__accumulated_time = np.float64(time.time()) - self.__last_frame_time

        return face_info


    def __is_ready(self):
        return self.__accumulated_time > self.__stabilize_time
    
    
    def __is_first_step(self):
        return self.__last_frame_time == np.float64(0.)
